...
sp_mat dydxFuncReal(const vec& p, const vec& q, const vec& r, int nx, int ny, double dx, double dy)
{
	int ng = nx * ny;

	vec a = p % join_cols(vec(nx + 1), r.rows(0, ng - (nx + 1) - 1)) % join_cols(vec(nx), 1 / q.rows(0, ng - nx - 1));
	vec b = -p % join_cols(vec(nx - 1), r.rows(0, ng - (nx - 1) - 1)) % join_cols(vec(nx), 1 / q.rows(0, ng - nx - 1));
	vec c = -p % join_cols(r.rows(nx - 1, ng - 1), vec(nx - 1)) % join_cols(1 / q.rows(nx, ng - 1), vec(nx));
	vec d = p % join_cols(r.rows(nx + 1, ng - 1), vec(nx + 1)) % join_cols(1 / q.rows(nx, ng - 1), vec(nx));
	...
	sp_mat s = spdiags(join_rows(d, c, b, a) / 4 / dx / dy,
		ivec{ -nx - 1, -nx + 1, nx - 1, nx + 1 }, ng, ng).st();
