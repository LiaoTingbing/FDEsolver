...
sp_mat dxdyFuncReal(const vec& p, const vec& q, const vec& r, int nx, int ny, double dx, double dy)
{
	...
	vec a = p % join_cols(vec(nx + 1), r.rows(0, ng - nx - 2)) % join_cols(vec(1), 1 / q.rows(0, ng - 2));
	vec b = -p % join_cols(vec(nx - 1), r.rows(0, ng - nx)) % join_cols(1.0 / q.rows(1, ng - 1), vec(1));
	vec c = -p % join_cols(r.rows(nx - 1, ng - 1), vec(nx - 1)) % join_cols(vec(1), 1 / q.rows(0, ng - 2));
	vec d = p % join_cols(r.rows(nx + 1, ng - 1), vec(nx + 1)) % join_cols(1 / q.rows(1, ng - 1), vec(1));
