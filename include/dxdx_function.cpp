...
void dxdxFunction(sp_mat& diagArr, ivec& diagIndex, const vec& p, const vec& q, const vec& r, int nx, int ny, double dx, double dy)
{
	int ng = nx * ny;
	vec a = p % join_cols(vec(1), r.rows(0, ng - 2))
		/ (join_cols(vec(1), q.rows(0, ng - 2)) + q);
	...
	diagArr = join_rows(a, b, c);
	diagIndex = { -1 , 0 , 1 };
}

 
void dxdxFunction(sp_mat& diagArr, ivec& diagIndex, const vec& p, const vec& q, const vec& r, const vec& x, const vec& y)
{
}
