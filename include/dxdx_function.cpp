...
sp_cx_mat dxdxFunc(const cx_vec& p, const cx_vec& q, const cx_vec& r, int nx, int ny, double dx, double dy)
{
	...
	int ng = nx * ny;
	cx_vec a = 1/p % join_cols(cx_vec(1), r.rows(0, ng - 2))
	...
	cx_vec c = 1/p % join_cols(r.rows(1, ng - 1), cx_vec(1))
...
sp_mat dxdxFunc(const vec& p, const vec& q, const vec& r, int nx, int ny, double dx, double dy)
{
	int ng = nx * ny;
	vec a = 1/p % join_cols(vec(1), r.rows(0, ng - 2))
	...
	vec c = 1/p % join_cols(r.rows(1, ng - 1), vec(1))
	...
	return spdiags(join_rows(c, b, a) * 2 / dx / dx,
		ivec{ -1,0,1 }, ng, ng).st();
}
